...
마지막으로 **산출물과 소스를 MYBOX 로 넘긴다.** 이게 NAS 백업으로 가는 유일한 입구다.

```bash
python scripts/archive-to-mybox.py             # 최신 릴리스. 특정 판이면 뒤에 v0.6.1
```

**이 걸음을 빼면 백업에 안 남는다.** 설치 파일은 GitHub Actions 안에서 만들어져 이 기계에
존재한 적이 없고, 소스는 `D:\dev\IceCmd` 인데 백업 엔진이 보는 곳은 `D:\Naver MYBOX` 다.
둘을 잇는 단계가 없어서 **0.6.0·0.6.1 두 판이 NAS 에 통째로 빠졌다**(2026-08-13 일감 #11).
동기화는 없는 파일을 가져올 수 없다.

그래서 릴리스를 낼 때마다 위 세 단계를 같이 밟는다.
